...
<div class = "main" id="testwjotMain" >
 	<div class = "search-container">
 	<form class="FindData-form" id = "FindData-form">
 		<div class=" search-top">外键测试管理11</div>
					<div class = "searchItem">名称:<input type="text" class="search-input" name="name"></div>
				<div class = "searchItem">testwjId:<select class="testwj  search-input" name="testwjId"  ></select></div>
 ...
 <shiro:hasPermission name="user:update">
  ...
  <shiro:hasPermission name="user:show">
   ...
   <shiro:hasPermission name="user:excel">
	...
	<table cellspacing="0" cellpadding="0" class="demo-table" id="testwjot-table">
			<thead>
				<tr>
...
(function($, exports) {
	var TestwjotModule = new exports.Modle("testwjotMain","testwjotmanagement","edittestwjot","testwjot-table","testwjot");
	//debugger;
	//构建获取的每一行
	TestwjotModule.buildHtmlBody = function(date){
		var htm = "";
    	for ( var i = 0; i < date.length; i++) {
    		htm += ("<tr data='' id='"+date[i].id+"' class='' ><td><input type='checkbox' class='checkboxId'></td><td class='show-entity'>"+(i+1)+"</td>"
	          	+ "<td>"+(date[i].name || "")+"</td>"
		      	+ "<td>"+(date[i].testwjName || "")+"</td>"
    		+"<td><input type='button' class='delete'></td></tr>");
		}
    	return htm;
	};
	TestwjotModule.initExcel({url:"testwjotmanagement",findFrom:"FindData-form",mailClassName:"export-main",data:[
																			{title:"名称",value:"name"},
																			{title:"testwjId",value:"testwjName"},
		                                                                    ]});
	TestwjotModule.init();
	window.Modules.findListAction("testwj");
	
	window.Modules.setModle("testwjot",TestwjotModule);
